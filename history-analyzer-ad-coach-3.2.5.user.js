...
// @version      3.2.5
// @description  History Analyzer - AD Coach - by KnuSpersTV mit robuster DE/GB-Übersetzung, stabiler Diagrammauswahl und responsiver Match-Entwicklung für die sichtbare Autodarts Visit History.
...
(() => {
  ...
  function normalizeSavedMatch(match) {
    if (!match || typeof match !== 'object') return null;
    if (!match.matchId || !Array.isArray(match.legs) || !Array.isArray(match.players)) return null;
    return { ...match, schemaVersion: STORAGE_SCHEMA_VERSION };
  }

  function extractSavedMatches(value) {
    const rawMatches = Array.isArray(value)
      ? value
      : value && Array.isArray(value.matches)
        ? value.matches
        : [];
    return rawMatches.map(normalizeSavedMatch).filter(Boolean);
  }
  ...
  function loadSavedMatches() {
    try {
      ...
      const matches = extractSavedMatches(value);
      const isLegacyArray = Array.isArray(value);
      const isUnversionedEnvelope = value && !Array.isArray(value) && value.matches && value.schemaVersion !== STORAGE_SCHEMA_VERSION;
      if (isLegacyArray || isUnversionedEnvelope) writeSavedMatches(matches);
      return matches;
    } catch (_) {
      return [];
    }
  ...
  function writeSavedMatches(matches) {
    try {
      const normalized = extractSavedMatches(matches);
  ...
  function exportSavedMatches() {
    const blob = new Blob([JSON.stringify({ schemaVersion: STORAGE_SCHEMA_VERSION, version: 1, exportedAt: new Date().toISOString(), matches: loadSavedMatches() }, null, 2)], { type: 'application/json' });
  ...
  async function importSavedMatches(file) {
    try {
      ...
      const valid = extractSavedMatches(parsed);
      ...
      for (const match of valid) byId.set(match.matchId, { ...byId.get(match.matchId), ...match, schemaVersion: STORAGE_SCHEMA_VERSION });
  ...
  function developmentMetricOptions() {
    const english = currentLanguage === 'en';
    const options = english
      ? [
          ['average', 'Visit average'],
          ['officialThreeDart', 'Official 3-dart average'],
          ['strongShare', 'Visits above 95 (%)'],
          ['weakShare', 'Visits up to 45 (%)'],
          ['t19Share', 'T19 share (%)'],
          ['t20Share', 'T20 share (%)'],
          ['checkoutRate', 'Checkout rate (%)'],
          ['legWinRate', 'Leg win rate (%)']
        ]
      : [
          ['average', 'Ø Aufnahme'],
          ['officialThreeDart', '3-Dart-Average offiziell'],
          ['strongShare', 'Aufnahmen über 95 (%)'],
          ['weakShare', 'Aufnahmen bis 45 (%)'],
          ['t19Share', 'T19-Anteil (%)'],
          ['t20Share', 'T20-Anteil (%)'],
          ['checkoutRate', 'Checkoutquote (%)'],
          ['legWinRate', 'Leggewinnquote (%)']
        ];
    return options.map(([value, label]) => `<option value="${value}"${developmentMetric === value ? ' selected' : ''}>${label}</option>`).join('');
  }

  function developmentMetricLabel() {
    const english = currentLanguage === 'en';
    const labels = english
      ? { average: 'Visit average', officialThreeDart: 'Official 3-dart average', strongShare: 'Visits above 95 (%)', weakShare: 'Visits up to 45 (%)', t19Share: 'T19 share (%)', t20Share: 'T20 share (%)', checkoutRate: 'Checkout rate (%)', legWinRate: 'Leg win rate (%)' }
      : { average: 'Ø Aufnahme', officialThreeDart: '3-Dart-Average offiziell', strongShare: 'Aufnahmen über 95 (%)', weakShare: 'Aufnahmen bis 45 (%)', t19Share: 'T19-Anteil (%)', t20Share: 'T20-Anteil (%)', checkoutRate: 'Checkoutquote (%)', legWinRate: 'Leggewinnquote (%)' };
    return labels[developmentMetric] || labels.average;
  }

  function developmentChartData(currentMatch, player) {
    ...
    return matches.map((match, index) => {
      ...
      const wonLegs = (item.match.legs || []).filter(leg => (leg.visits || []).some(visit => visit.player === player && visit.remaining === 0)).length;
      const legCount = item.match.legs?.length || 0;
      const value = {
        average: item.average,
        officialThreeDart: item.scoringStats.officialThreeDartAverage,
        strongShare: item.visits.length ? item.thresholds.over95 / item.visits.length * 100 : 0,
        weakShare: item.visits.length ? item.thresholds.atMost45 / item.visits.length * 100 : 0,
        t19Share: item.darts.length ? item.fields[19].triple / item.darts.length * 100 : 0,
        t20Share: item.darts.length ? item.fields[20].triple / item.darts.length * 100 : 0,
        checkoutRate: item.checkout.attempts ? item.checkout.successes / item.checkout.attempts * 100 : 0,
        legWinRate: legCount ? wonLegs / legCount * 100 : 0
      }[developmentMetric];
      return { label: match.matchId === currentMatch.matchId ? (currentLanguage === 'en' ? 'Current' : 'Aktuell') : matchLabel(match, index), value: Number.isFinite(value) ? value : 0, current: match.matchId === currentMatch.matchId };
    }).filter(Boolean);
  }

  function developmentChart(currentMatch, player) {
    const data = developmentChartData(currentMatch, player);
    const english = currentLanguage === 'en';
    if (data.length < 2) {
      return `<div class="adpc-warning">${english ? 'At least two matches are needed for the development chart.' : 'Für das Entwicklungsdiagramm werden mindestens zwei Matches benötigt.'}</div>`;
    }
    // Mehr Platz pro Match verhindert überlappende Beschriftungen auf der X-Achse.
    // Der Chart bleibt horizontal scrollbar, falls viele Matches vorhanden sind.
    // Alle Matches werden in ein responsives Diagramm eingepasst. Die SVG wird
    // proportional auf die verfügbare Panelbreite skaliert; es gibt keinen
    // horizontalen Scrollbereich mehr.
    const width = 720;
    const height = 290;
    ...
    const low = Math.max(0, spread ? minValue - spread * 0.12 : minValue - 5);
    const high = spread ? maxValue + spread * 0.12 : maxValue + 5;
    ...
    const points = data.map((point, index) => `${x(index).toFixed(1)},${y(point.value).toFixed(1)}`).join(' ');
    const grid = [0, 1, 2, 3, 4].map(index => {
      const value = low + (high - low) * (4 - index) / 4;
      const yy = y(value);
      return `<line x1="${pad.left}" y1="${yy.toFixed(1)}" x2="${width - pad.right}" y2="${yy.toFixed(1)}" class="adpc-chart-gridline"/><text x="${pad.left - 8}" y="${(yy + 4).toFixed(1)}" class="adpc-chart-axis-label">${value.toFixed(1)}</text>`;
    }).join('');
    // Keine langen Matchnamen auf der X-Achse: Jeder Datenpunkt erhält nur
    // einen kleinen Tick, die Achse selbst wird allgemein mit „Match“ beschriftet.
    const ticks = data.map((point, index) => {
      const xx = x(index);
      return `<line x1="${xx.toFixed(1)}" y1="${height - pad.bottom}" x2="${xx.toFixed(1)}" y2="${height - pad.bottom + 6}" class="adpc-chart-axis"/>`;
    }).join('');
    const xAxisTitle = `<text x="${((pad.left + width - pad.right) / 2).toFixed(1)}" y="${height - 18}" text-anchor="middle" class="adpc-chart-x-label">Match</text>`;
    const markers = data.map((point, index) => {
      const cx = x(index).toFixed(1);
      const cy = y(point.value).toFixed(1);
      const visualClass = point.current ? 'adpc-chart-point-current' : 'adpc-chart-point';
      const exactValue = point.value.toFixed(1);
      const title = `${point.label}: ${exactValue} – ${developmentMetricLabel()}`;
      // Unsichtbarer, größerer Trefferbereich macht die Punkte auch bei vielen
      // Matches und auf kleinen Bildschirmen zuverlässig anwählbar.
      return `<circle cx="${cx}" cy="${cy}" r="10" class="adpc-chart-hit-area" data-development-point="${index}" tabindex="0" role="img" aria-label="${escapeHtml(title)}"><title>${escapeHtml(title)}</title></circle><circle cx="${cx}" cy="${cy}" r="${point.current ? 5 : 3.5}" class="${visualClass}" pointer-events="none"/>`;
    }).join('');
    return `<div class="adpc-chart-controls"><label>${english ? 'Metric' : 'Kennzahl'} <select data-development-metric>${developmentMetricOptions()}</select></label><span class="adpc-chart-caption">${escapeHtml(developmentMetricLabel())}</span></div><div class="adpc-chart-wrap"><svg class="adpc-chart" viewBox="0 0 ${width} ${height}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${escapeHtml(developmentMetricLabel())}"><line x1="${pad.left}" y1="${height - pad.bottom}" x2="${width - pad.right}" y2="${height - pad.bottom}" class="adpc-chart-axis"/>${ticks}${grid}<polyline points="${points}" class="adpc-chart-line" fill="none"/>${markers}${xAxisTitle}</svg><div class="adpc-chart-tooltip" data-development-tooltip role="status" aria-hidden="true"></div></div>`;
  }
  ...
  function developmentTable(currentMatch, player) {
    ...
    if (!rows) return '';
    return `${developmentChart(currentMatch, player)}<div class="adpc-note">Die Tabelle enthält automatisch gespeicherte Matches. T19-/T20-Anteil = jeweilige Triple-Treffer geteilt durch alle Darts des Matches.</div><div class="adpc-development-scroll"><table><thead><tr><th>Match</th><th>Gespeichert</th><th>Gegner</th><th>Legs</th><th>Aufnahmen</th><th>Ø Aufnahme</th><th>Höchste</th><th>T19-Anteil</th><th>T20-Anteil</th><th>&gt;95</th><th>≤45</th></tr></thead><tbody>${rows}</tbody></table></div>`;
  }
  ...
  function showAnalyzerError(error) {
    try { console.error('[History Analyzer]', error); } catch (_) {}
    if (!output) return;
    const english = currentLanguage === 'en';
    const message = english
      ? 'The analyzer could not process the visible Visit History. Autodarts itself is not affected. Try again after the history has loaded.'
      : 'Der Analyzer konnte die sichtbare Visit History nicht verarbeiten. Autodarts selbst ist nicht betroffen. Versuche es erneut, nachdem der Aufnahmenverlauf geladen ist.';
    const retry = english ? 'Try again' : 'Erneut versuchen';
    output.innerHTML = `<div class="adpc-warning adpc-error" role="alert">${escapeHtml(message)}<br><button type="button" id="adpc-retry-error">${escapeHtml(retry)}</button></div>`;
    output.querySelector('#adpc-retry-error')?.addEventListener('click', runAnalysis);
  }

  function render(analysis) {
    try {
      renderUnsafe(analysis);
    } catch (error) {
      showAnalyzerError(error);
    }
  }

  function renderUnsafe(analysis) {
    if (!output) return;
    ...
    `);
    const chartTooltip = output.querySelector('[data-development-tooltip]');
    output.querySelectorAll('[data-development-point]').forEach(point => {
      const showChartTooltip = () => {
        if (!chartTooltip) return;
        const wrap = point.closest('.adpc-chart-wrap');
        if (!wrap) return;
        ...
        chartTooltip.style.left = `${Math.max(8, Math.min(wrapRect.width - 8, pointX))}px`;
        chartTooltip.style.top = `${Math.max(8, pointY - 8)}px`;
      ...
      point.addEventListener('pointerenter', showChartTooltip);
    ...
    output.querySelectorAll('[data-development-metric]').forEach(select => {
      select.addEventListener('change', () => {
        developmentMetric = select.value;
        render(lastAnalysis);
      });
    });
  ...
  function createPanel() {
    ...
    panel.id = PANEL_ID;
    panel.innerHTML = `
      <div class="adpc-head"><b data-adpc-static="title">${uiText('title')}</b><div class="adpc-head-actions"><button id="adpc-top" title="${uiText('topTitle')}" aria-label="${uiText('topAria')}">↑</button><button id="adpc-minimize" title="${uiText('minimize')}">−</button><button id="adpc-close" title="${uiText('close')}">×</button></div></div>
      <div class="adpc-body">
        <div class="adpc-controls">
          ...
          <button id="adpc-lang-de" data-language="de" title="Deutsch" aria-label="Deutsch">DE <span class="adpc-flag adpc-flag-de" aria-hidden="true"></span></button>
    ...
    style.textContent = `
      ...
      #${PANEL_ID} .adpc-language-active{background:#3b4b68;border-color:#55b6a7}
      ...
      #${PANEL_ID} .adpc-error button{margin-top:7px}
      ...
      #${PANEL_ID} .adpc-chart-controls{display:flex;flex-wrap:wrap;gap:8px;align-items:center;margin:0 0 8px}
      #${PANEL_ID} .adpc-chart-caption{color:#9eabc0;font-size:11px}
      #${PANEL_ID} .adpc-chart-wrap{position:relative;overflow:hidden;background:#20283a;border-radius:7px;padding:8px;margin-bottom:6px}
      #${PANEL_ID} .adpc-chart{display:block;width:100%;max-width:100%;height:auto;aspect-ratio:720 / 290}
      #${PANEL_ID} .adpc-chart-hit-area{fill:transparent;stroke:transparent;cursor:crosshair;outline:none}
      #${PANEL_ID} .adpc-chart-hit-area:focus{stroke:#fff;stroke-width:2;stroke-dasharray:3 2}
      #${PANEL_ID} .adpc-chart-tooltip{position:absolute;z-index:5;display:none;max-width:min(260px,calc(100% - 16px));padding:6px 8px;border:1px solid #687792;border-radius:5px;background:#111722;color:#f2f5fa;font-size:12px;line-height:1.3;white-space:normal;pointer-events:none;box-shadow:0 4px 14px rgba(0,0,0,.35)}
      #${PANEL_ID} .adpc-chart-tooltip.is-visible{display:block}
      #${PANEL_ID} .adpc-chart-legend{display:flex;flex-wrap:wrap;gap:4px 10px;margin:0 2px 10px;color:#9eabc0;font-size:11px;line-height:1.35}
      #${PANEL_ID} .adpc-chart-legend-item{white-space:normal}
      #${PANEL_ID} .adpc-chart-legend-current{color:#f1c36a}
      #${PANEL_ID} .adpc-chart-gridline{stroke:#39435a;stroke-width:1}
      #${PANEL_ID} .adpc-chart-axis{stroke:#6f7c96;stroke-width:1}
      #${PANEL_ID} .adpc-chart-line{stroke:#55b6a7;stroke-width:3;stroke-linejoin:round;stroke-linecap:round}
      #${PANEL_ID} .adpc-chart-point{fill:#55b6a7;stroke:#151923;stroke-width:2}
      #${PANEL_ID} .adpc-chart-point-current{fill:#e2a84b;stroke:#fff;stroke-width:2}
      #${PANEL_ID} .adpc-chart-axis-label,#${PANEL_ID} .adpc-chart-x-label{fill:#9eabc0;font:11px Arial,sans-serif}
  ...
  function scheduleAnalysis(records) {
    // Eigene Render-Mutationen des Analyzers dürfen keine erneute Analyse
    // auslösen. Sonst wird ein geöffnetes Select regelmäßig ersetzt und klappt
    // sofort wieder zu.
    if (panel && records?.length && records.every(record => {
      if (panel.contains(record.target)) return true;
      const nodes = [...record.addedNodes, ...record.removedNodes];
      return nodes.length > 0 && nodes.every(node => node.nodeType === 1 && (node === panel || panel.contains(node)));
    })) return;
    clearTimeout(parseTimer);
    parseTimer = setTimeout(() => {
      if (!panel || !document.body.contains(panel)) return;
      const active = document.activeElement;
      // Während einer Bedienung des Panels nicht durch externe DOM-Änderungen
      // neu rendern; dadurch bleiben Dropdowns und Tastaturauswahl stabil.
      if (active && panel.contains(active) && active.matches('select, input, button, textarea, [contenteditable="true"]')) return;
      runAnalysis();
    }, 700);
  }
